...
import openai
from cleaning import clean_code_solution
...
async def prompt_openai(prompt: str, kidx=0):
    ...
    code_sol = clean_code_solution(sol)
    print("onto", kidx)
    return code_sol
...
async def experiment(dataset, results_dir: str, experiment_name: str, model: str, k=10, batch_size=5, load_from_file=False):
    ...
    for idx, code_sols in enumerate(batch_code_sols):
        ...
        for cidx, code_sol in enumerate(code_sols):
            # check if code sol includes function signature
            if code_sol.find(f"def {task['entry_point']}") == -1:
                code_sol = task["prompt"] + "\n" + code_sol
            ...
            if passed:
                ...
                for k_metric in pass_k_metrics:
                    # remember 0-index, so cidx < k_metric, instead of <=, lol
                    if cidx < k_metric:
...
async def main():
    ...
    experiment_name = 'pipeline' if use_framework else 'baseline'
    await experiment(
        dataset,
        results_dir=f"./results/{experiment_name}/{dataset_name}/{model}",
        experiment_name=f"{dataset_name} ({experiment_name}) ({model})",
        model=model,
        k=10,
        load_from_file=True
    )
